  ...
  const res = useMemo(
    () =>
      simulateSip({
        ...
        annualReturn,
        years,
        stepUpPct: stepUpOn ? stepUpPct : 0,
        existingCorpus,
      }),
    [monthlyAmount, annualReturn, years, stepUpOn, stepUpPct, existingCorpus]
  );

  const withoutStepUp = useMemo(
    () => simulateSip({ monthlyAmount, annualReturn, years, existingCorpus }),
    [monthlyAmount, annualReturn, years, existingCorpus]
  );
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <Panel title="Your SIP">
        ...
        <Row
          label="Already invested elsewhere"
          money
      ...
      <Panel title="Customise">
        <div className="mb-4 flex flex-wrap gap-2">
          <Chip onClick={() => { setStepUpOn(true); setStepUpPct(10); }}>Step up 10%/yr</Chip>
          <Chip onClick={() => { setStepUpOn(true); setStepUpPct(15); }}>Step up 15%/yr</Chip>
          <Chip onClick={() => setStepUpOn(false)}>No step-up</Chip>
        </div>
        <Toggle on={stepUpOn} set={setStepUpOn}>Step up SIP every year</Toggle>
        {stepUpOn && (
          <div className="pt-3">
            <Row
              ...
              step={1}
              suffix="% / yr"
              hint={`Your monthly SIP grows to about ₹${inr(res.finalMonthlyAmount)} in the final year`}
            />
          </div>
        )}
      </Panel>
      ...
      <p className="px-1 text-[11.5px] leading-relaxed text-ink-700/50">
        Assumes returns compound monthly at a constant rate and each SIP instalment is invested on the 1st of the
        month. Real fund returns vary year to year — use this as a planning guide, not a guarantee.
      </p>
    </div>
  );
}
